...
def _run_ci_cmd(args) -> None:
    ...
    # --- Coverage floor gate (CI) -------------------------------------------
    min_coverage = pol.get("min_coverage")
    if min_coverage is not None:
        from pipeline.test_generator import compute_coverage_delta
        current_cov, _, cov_delta, _ = compute_coverage_delta(inputs, args.target)
        if current_cov is not None and current_cov < float(min_coverage):
            print(
                f"\n✖  COVERAGE GATE FAILED — {current_cov:.1f}% < {min_coverage}% (min_coverage policy)\n"
                f"   delta this run: "
                + (f"{cov_delta:+.1f}%" if cov_delta is not None else "first run")
            )
            sys.exit(2)
        elif current_cov is not None and cov_delta is not None:
            sign = "+" if cov_delta >= 0 else ""
            print(f"Coverage: {current_cov:.1f}%  ({sign}{cov_delta:.1f}% this run)  [floor: {min_coverage}%]")
...
def main() -> None:
    ...
    check_cmd.add_argument(
        ...
        dest="generate_tests",
        help=(
            "After analysis, write runnable tests for high-risk uncovered paths to FILE. "
            "Detects your test framework (pytest/jest/go test/…) automatically. "
            "Tracks coverage trend — warns if coverage drops vs last run. "
            "Tests are flagged '# generated by aicritic' and are NOT auto-committed."
        ),
    )
    check_cmd.add_argument(
        "--auto-commit-tests",
        action="store_true",
        default=False,
        dest="auto_commit_tests",
        help="Must be set explicitly to allow generated tests to be committed (default: off).",
    )
    ...
    # Optional: test generation — find high-risk uncovered paths, write runnable tests
    test_generator_result = None
    if args.generate_tests:
        from pipeline.test_generator import run_test_generator
        from report.formatter import print_test_generator
        import policy as _tg_policy
        _tg_pol = _tg_policy.load(args.target)
        console.print("\n[dim]  Running Test Generator…[/dim]")
        test_generator_result = run_test_generator(
            inputs=inputs,
            critic_result=critic_display,
            target=args.target,
            output_file=args.generate_tests,
            policy=_tg_pol,
        )
        print_test_generator(test_generator_result)

        # Coverage CI gate: fail if below policy floor
        if test_generator_result.get("policy_violation"):
            floor = test_generator_result.get("policy_floor", "?")
            curr  = test_generator_result.get("overall_coverage", "?")
            console.print(
                f"[bold red]✖ Coverage gate failed:[/bold red] "
                f"{curr:.1f}% < {floor}% (min_coverage policy)\n"
                f"[dim]Add min_coverage to .aicritic-policy.yaml to configure this threshold.[/dim]"
            )
            # Save report before exiting
            save_markdown(
                args.target, analyst_filtered, checker_filtered, critic_display,
                args.output, explainer=explainer_result,
                pattern_advisor=pattern_advisor_result,
                test_generator=test_generator_result,
            )
            sys.exit(2)
    ...
    if args.json_output:
        jpath = save_json(args.target, analyst_filtered, checker_filtered, critic_display, args.json_output, explainer=explainer_result, pattern_advisor=pattern_advisor_result, test_generator=test_generator_result)
        console.print(f"[bold green]JSON saved:[/bold green] {jpath}\n")

    # Optional: HTML report
    if args.html_output:
        hpath = save_html(args.target, analyst_filtered, checker_filtered, critic_display, args.html_output, explainer=explainer_result, pattern_advisor=pattern_advisor_result, test_generator=test_generator_result)
